...
namespace Jorteck.ChatTools
{
  ...
  internal sealed class ChatCommandService
  {
    ...
    private readonly ICommandListProvider commandListProvider;
    private readonly HelpCommand helpCommand;
    private readonly string helpCommandText;

    public ChatCommandService(ICommandListProvider commandListProvider, ConfigService configService, HelpCommand helpCommand)
    {
      if (configService.Config.CommandPrefixes == null || configService.Config.CommandPrefixes.Count == 0)
      {
      ...
      helpCommandText = $"{configService.Config.CommandPrefixes[0]}{helpCommand.Command}".ColorString(ColorConstants.Orange);
    ...
    private void OnChatMessageSend(OnChatMessageSend eventData)
    {
      ...
      foreach (string commandPrefix in configService.Config.CommandPrefixes)
      {
